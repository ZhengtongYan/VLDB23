            ...
            for i in range(3, 6):
                ...
                for j in range(20):
                    ...
                    if aggregate_data:
                        ...
                        save_to_csv([at, graph_type, i, j, valid_ratio, optimal_ratio], path, 'QPU_results.txt')
                if aggregate_data:
                    mean_valid_ratio = get_rounded_val(np.mean(valid_ratios))
                    mean_opt_ratio = get_rounded_val(np.mean(opt_ratios))
                    save_to_csv([at, graph_type, i, mean_valid_ratio, mean_opt_ratio], path, 'aggregated_QPU_results.txt')
